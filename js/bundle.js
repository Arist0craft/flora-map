...
function buildSearhFormLayout() {
  const MySearchFormLayout = ymaps.templateLayoutFactory.createClass(
    `
    <div class="map-search">
      ...
      <input id="map-suggest" type="search" placeholder="{{ options.placeholderContent }}">
    </div>
    {% include options.popupLayout %}
    `,
    {
      build: function () {
        this._elements = { test: "test" };
      },
    }
  );

  return MySearchFormLayout;
}
...
function getMySearchControl(geoPoints) {
  ...
  const mySearchControl = new ymaps.control.SearchControl({
    options: {
      ...
      // normalLayout: buildSearhFormLayout(),
    },
  });
